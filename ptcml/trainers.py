...
from .loss import TMCLS, TMC, SoftTripletLoss, CrossEntropyLabelSmooth
from .utils.meters import AverageMeter


class PTCMLTrainer(object):
    def __init__(self, model, rts_score, num_class=500, num_part=6, beta=0.5, tmcls_epoch=5):
        super(PTCMLTrainer, self).__init__()
        self.model = model
        self.rts_score = rts_score

        self.num_class = num_class
        self.num_part = num_part
        self.tmcls_epoch = tmcls_epoch

        self.criterion_tmc = TMC(lam=beta).cuda()
        self.criterion_tmcls = TMCLS().cuda()
    ...
    def train(self, epoch, train_dataloader, optimizer, print_freq=1, train_iters=200):
        ...
        for i in range(train_iters):
            ...
            inputs, targets, rts = self._parse_data(data)
            ...
            # loss
            loss_gce = self.criterion_tmc(logits_g, logits_p, targets, rts)
            loss_tri = self.criterion_tri(emb_g, targets)

            loss_pce = 0.
            if self.num_part > 0:
                if epoch >= self.tmcls_epoch:
                    for part in range(self.num_part):
                        loss_pce += self.criterion_tmcls(logits_p[:, :, part], targets, rts[:, part])
                else:
                    for part in range(self.num_part):
    ...
    def _parse_data(self, inputs):
        ...
        rts = self.rts_score[idxs]
        return imgs.cuda(), pids.cuda(), rts.cuda()


class PTCMLTrainerCAM(object):
    def __init__(self, model, rts_score, memory, memory_p, num_class=500, num_part=6, beta=0.5, tmcls_epoch=5, lam_cam=0.5):
        super(PTCMLTrainerCAM, self).__init__()
        self.model = model
        self.rts_score = rts_score
        self.memory = memory
    ...
    def train(self, epoch, train_dataloader, optimizer, print_freq=1, train_iters=200):
        ...
        for i in range(train_iters):
            data = train_dataloader.next()
            inputs, targets, cams, rts = self._parse_data(data)
    ...
    def _parse_data(self, inputs):
        ...
        rts = self.rts_score[idxs]
        return imgs.cuda(), pids.cuda(), cids.cuda(), rts.cuda()
